...
int main()
{
	...
	// destruction time.
	{
		vector<Store> auxiliaryStores(stores);
		vector<Store *> auxiliaryStorePtrs;
		
		for (int i = 0; i < auxiliaryStores.size(); i++) {
			auxiliaryStorePtrs.push_back(&auxiliaryStores[i]);
		}

		sort(auxiliaryStorePtrs.begin(), auxiliaryStorePtrs.end(), compareStorePtr);
		
		for (int i = 0; i < stores.size(); i++) {
			stores[i] = (*auxiliaryStorePtrs[i]);
		}
